...
Fs_int = Fs*inter;
T_in = 1/Fs;
L = size(recieved,1); % Length of recieved signal         
t =  ((0:L-1)*T_in);
t = t';% Time vector

recieved_I = recieved.*cos(2*pi*(f - Freq_pll_2)*t);
...
recieved_Q = recieved.*sin(2*pi*(f - Freq_pll_2)*t);
clear recieved
clear t
...
for i = 1:(size(recieved_Q_filterred_int,1))
    m6(i) = (sum(recieved_Q_filterred_sof(1+(i-1):((i-1)+26*number_s*inter)).*sof_reshaped))^2;
end
...
for i = 1:(size(recieved_I_filterred_int,1))
    m7(i) = (sum(recieved_I_filterred_sof(1+(i-1):((i-1)+26*number_s*inter)).*sof_reshaped))^2;
end
...
a = double(a);
start = find(a == max(a(1:48000*10)));
clear a

recieved_I_filterred_int_zero = recieved_I_filterred_int(start+5:start-1+22166*number_s*inter*numpackets);
clear recieved_I_filterred_int

recieved_Q_filterred_int_zero = recieved_Q_filterred_int(start+5:start-1+22166*number_s*inter*numpackets);
